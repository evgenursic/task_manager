...
const FALLBACK_OWNER_EMAIL = "demo@taskflow.local";

/**
 * Temporary owner fallback used until auth-scoped ownership is implemented.
 * @param {PrismaClient} client
 */
async function resolveFallbackOwnerId(client) {
  ...
  const createdOwner = await client.user.create({
    data: {
      name: "Demo User",
...
export async function createTask(input, client = db) {
  try {
    ...
    const ownerId = await resolveFallbackOwnerId(client);
    const data = {
      ...parsed,
      ownerId,
    };
 ...
 */
export async function getTaskReminderSummary(input = {}, client = db) {
  try {
